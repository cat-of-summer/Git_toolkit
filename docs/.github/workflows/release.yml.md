...
Формат тега:
- `vX.Y.Z` — обычный тег; целевая ветка (Environment) определяется автоматически по коммиту
- `{branch}/vX.Y.Z` — тег с явной привязкой к ветке (например, `main/v1.2.3`). Ветка берётся из префикса однозначно (без автоопределения); если такой ветки нет — workflow падает. Префикс `{branch}/` вырезается из версии, поэтому в заголовке релиза и в версии пакета/образа используется только `vX.Y.Z`
...
1. **Resolve & validate tag** — берёт тег из `github.ref_name`; если он содержит `/`, делит по
   **последнему** `/` на префикс-ветку (`{branch}`, может содержать `/`, напр. `release/1.x`) и
   ...
   `^v[0-9]+(\.[0-9]+)*$` и падает с понятной ошибкой, если она не подходит; кладёт в outputs
   `ref` (полный тег), `tag` (версия) и `branch_explicit` (префикс-ветка или пусто)
...
| `PER_BRANCH_PACKAGE` | `false` | Если `true` — образ собирается **отдельным на каждую ветку**: к имени добавляется суффикс `-<branch>` (`owner/repo` → `owner/repo-<branch>`). Позволяет держать разные проекты на разных ветках в разных образах без коллизий. По умолчанию (`false`) — один образ репозиторного уровня |
...
4. **Resolve image ref & version** — формирует полное имя образа `<registry>/<image>` (lowercase) и версию из очищенного тега `needs.prepare.outputs.tag` (`v1.2.3` → `1.2.3`, валидация формата). Если `PER_BRANCH_PACKAGE=true`, к имени образа добавляется суффикс `-<branch>` (например, `owner/repo-main`); ветка берётся из `needs.prepare.outputs.branch` (явная из тега либо авто-определённая, уже санитизированная `/`→`-`)
...
Workflow определяет целевую ветку и использует её имя как GitHub Environment. Это позволяет иметь разные настройки сборки и разные файлы релиза для разных веток (например, отдельные конфиги для `main` и `develop`). Есть два способа:

- **Автоопределение (тег `vX.Y.Z`)** — ветка ищется по коммиту тега через `git branch -r --contains`, берётся первая подходящая. Просто, но при коллизии (коммит есть в нескольких ветках) выбор неоднозначен.
- **Явная привязка (тег `{branch}/vX.Y.Z`)** — ветка берётся из префикса тега однозначно, автоопределение не выполняется. Наличие ветки проверяется на remote; если её нет — workflow падает. Пример: `main/v1.2.3` → Environment `main`. Префикс вырезается, поэтому заголовок релиза (`main release v1.2.3`) и версия пакета/образа (`1.2.3`) не дублируют ветку.
...
**Отдельный образ на ветку.** Если на разных ветках живут по сути разные проекты, включи
`PER_BRANCH_PACKAGE=true` в нужном Environment — Docker-образ получит суффикс `-<branch>`
(`owner/repo-main`, `owner/repo-develop`), и образы веток не будут пересекаться. Поскольку ветку
достоверно даёт только явный тег, для этого режима **рекомендуется** пушить теги в форме
`{branch}/vX.Y.Z`: при плоском теге суффикс возьмётся из авто-определённой (возможно неточной)
ветки.
...
# отдельный образ на каждую ветку (owner/repo-<branch>):
PER_BRANCH_PACKAGE=true
```
```yaml
# docker-compose.yml
services:
