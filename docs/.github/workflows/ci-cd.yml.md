...
| `BUILD_CONTEXT` | `.` | Контекст сборки; сюда же распаковываются результаты `ci` |
...
Нужен `Dockerfile`. Результаты `ci` уже распакованы в `BUILD_CONTEXT`, поэтому в образе достаточно
