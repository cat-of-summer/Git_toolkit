...
Формат `<инструмент><разделитель><версия>` через запятую. Разделитель — `:` или `@` (как
удобнее; `@` — родной синтаксис mise). Версию можно опустить — будет `latest`.
...
> Python ставится **готовым** бинарником (python-build-standalone), а не компилируется из
> исходников — быстро и без системных зависимостей.
